...
namespace ETrade.Repos.Concretes
{
    public class ProductsRep<T> : BaseRepository<Products>, IProductsRep where T : class
    {
        ...
        {
          return  Set().Where(x=>x.Id == Id).Include(x=>x.Vat).FirstOrDefault(); //Find ile yapmadık çünkü key istiyor ve include yapamıyoruz
                                            //include u ekleyince lazy loading oldu
        }
